...
UINT numElements)
{
	...
	if (FAILED(hr))
	{
		printf("Create InputLayout Failed!\r\n");
		return hr;
	}
...
HRESULT VertexShaderClass::SetParam(ID3D11Device* device, ID3D11DeviceContext* context, float _param[])
{
	va_list ls;
	float temp;
	memcpy(param, _param, sizeof(float)* 8);
	D3D11_MAPPED_SUBRESOURCE myresource;
	context->Map(paramBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &myresource);
	float* dptr = (float*)myresource.pData;
	memcpy(dptr, param, sizeof(float)* 8);
	context->Unmap(paramBuffer, 0);
	context->VSSetConstantBuffers(9, 1, &paramBuffer);
	return S_OK;
}